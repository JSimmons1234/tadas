...
class Application  extends LoginController {
  ...
  def displayProjectsRootDirectory: Result = {
  ...
  def listProfiles = Action { implicit request =>
    ...
    Ok(views.html.profileList(Profile.profiles, Profile.createProfileForm))
  }
  ...
  def createProfile = Action { implicit request =>
    ...
    val formValidationResult = Profile.createProfileForm.bindFromRequest
    ...
    formValidationResult.fold({ formWithErrors =>
      BadRequest(views.html.profileList(Profile.profiles, formWithErrors))
    }, { profile =>
      ...
      Redirect(routes.Application.listProfiles)
    })
  }
